    ...
    internal class List : IScraper
    {
        ...
        public async Task Start(string input)
        {
            ...
            if (this.Assets.Count > 0)
            {
                ...
                await File.WriteAllLinesAsync(Path.Combine(Config.OutputPath, "index.txt"), this.Assets.Select(x => x.Value));
            }
        ...
        private void DownloaderOnSuccess(object sender, DownloaderSuccessEventArgs e)
        {
            ...
            {
                path = Path.Combine(Config.OutputPath, e.Input);
                Directory.CreateDirectory(path);
                path = FileWriter.ConstructPath(path, $"{e.Input}-v{e.Version}");
            }
            else
